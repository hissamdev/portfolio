...
export default function HeroAuditButton() {
    ...
    return (
        <>
            ...
            <Modal showModal={showModal} onClose={() => {setShowModal(false); setLoading(true)}}>
                <div className="m-auto p-10 w-[800px] h-fit bg-white rounded-2xl">
                    <div className="flex justify-between">
                        <div className="flex gap-5">
                            <Paperclip size={24} />
                            <div>
                                <h3 className="text-2xl font-bold">Ahmed_Hissam_Resume.pdf</h3>
                                <p className="text-slate-500">Full Stack Developer</p>
                            </div>
                        </div>

                        <div>
                            <button onClick={() => {setShowModal(false); setLoading(true)}} className="p-2 transition-colors bg-gray-100 hover:bg-gray-200 active:bg-red-300 active:text-white rounded-lg cursor-pointer">
                                <X size={24} />
                            </button>
                        </div>
                    </div>
                        ...
                        {loading ?
                            <div className="w-full h-[400px] flex justify-center items-center border-2 border-dashed border-slate-300 bg-slate-100 rounded-2xl">
                                <div>
                                    ...
                                    <h4 className="pt-4 text-lg text-center font-bold">Downloading Resume...</h4>
                                    <p className="text-center">Preparing your high-resolution copy</p>
                                </div>
                            </div>
                        :
                            <div className="p-7 w-full h-fit flex flex-col justify-center items-start border-2 border-dashed border-slate-300 bg-slate-100 rounded-2xl">
                                    ...
                                    <div className="mt-4 w-full bg-white rounded-lg">
                                        <div className="h-120">
                                            <iframe src="/Ahmed-Hissam-Resume.pdf#toolbar=0&navpanes=0&scrollbar=0" className="w-full h-full"/>
                                        </div>
                                    </div>
                            </div>
                        }
